...
def rA(dielectricIndex, radiiIndex):  # Radii Adjustment
    ...
    return (dielectricArray[:, dielectricIndex-1]*2*sc.pi*radii[radiiIndex-1]*N_m)/lamda
...
#############################################################
#                                                           #
#                 2017 Ladutenkoa et al.                    #
#           Their second publication on multiayer           #
#                       Mie Theory                          #
#                                                           #
...
def Dn1(n, z):
    return Dpn(n, z)/pn(n, z)


def Dn3(n, z):
    return Dzn(n, z)/zn(n, z)


def U(n, z):
    return Dn1(n, z)-Dn3(n, z)
...
def anl(n, l):
    if (l == 0 or l == numLayers + 1):
        return 0
    else:
        return ((Dn1(n, rA(l, l))*T1(n, l, rA(l+1, l)))+(T3(n, l, rA(l+1, l))*(dielectricArray[:, l-1]/dielectricArray[:, l])))/(zn(n, rA(l, l))*U(n, rA(l,l)))
...
def bnl(n, l):
    if (l == 0 or l == numLayers + 1):
        ...
        return ((Dn1(n, rA(l, l))*T2(n, l, rA(l+1, l))*(dielectricArray[:, l-1]/dielectricArray[:, l]))+(T4(n, l, rA(l+1, l))))/(zn(n, rA(l, l))*U(n, rA(l,l)))
...
def cnl(n, l):
    if (l == numLayers + 1):
        ...
        return ((Dn3(n, rA(l, l))*T2(n, l, rA(l+1, l))*(dielectricArray[:, l-1]/dielectricArray[:, l]))+(T4(n, l, rA(l+1, l))))/(pn(n, rA(l, l))*U(n, rA(l,l)))
...
def dnl(n, l):
    if (l == numLayers + 1):
        ...
        return ((Dn3(n, rA(l, l))*T1(n, l, rA(l+1, l)))+(T3(n, l, rA(l+1, l))*(dielectricArray[:, l-1]/dielectricArray[:, l])))/(pn(n, rA(l, l))*U(n, rA(l,l)))


def T1(n, l, z):
    return (anl(n, l+1)*zn(n, z))-(dnl(n, l+1)*pn(n, z))


def T2(n, l, z):
    return (bnl(n, l+1)*zn(n, z))-(cnl(n, l+1)*pn(n, z))


def T3(n, l, z):
    return (dnl(n, l+1)*Dn1(n, z)*pn(n, z))-(anl(n, l+1)*Dn3(n, z)*zn(n, z))
...
#         sumd = 0
#         for n in range(1, lastTerm+1):
#             sumd = sumd + (2*n+1)*(an(n, solid+1)+bn(n, solid+1)).real
